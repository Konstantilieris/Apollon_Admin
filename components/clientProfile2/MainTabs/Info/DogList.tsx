...
import { getAgeYandM } from "@/lib/utils";
...
const DogList = ({ clientId, dogs }: { clientId: string; dogs: Dog[] }) => {
  ...
  return (
    <Card className=" ml-8 w-full border-0">
      ...
      <CardContent>
        {dogs.map((dog, index) => {
          if (!dog) return null; // Skip if dog is undefined or null
          const { years, months } = getAgeYandM(dog.birthdate);
          return (
            <div
              key={index}
            ...
            >
              ...
              <p className="text-lg tracking-wide">
                Ηλικία: {years} {years === 1 ? "έτος" : "έτη"} και {months} 
                {months === 1 ? "μήνα" : "μήνες"}
              </p>
