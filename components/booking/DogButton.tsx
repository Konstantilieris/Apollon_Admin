...
const DogButton = ({ dog, bedName, roomName, setBed, bed }: DogButtonProps) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const handleAddDogToBed = () => {
    if (!searchParams.has(dog._id)) {
      const cleanRoomName = roomName.trim().replace(/\s+/g, "");
      const cleanBedName = bedName.trim().replace(/\s+/g, "");
      setBed({
        ...bed,
      ...
      const newUrl = formUrlQuery({
        ...
        key: `${cleanRoomName}_${cleanBedName}`,
        value: dog._id,
      });
